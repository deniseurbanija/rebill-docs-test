...
import { useFilteredNav } from '@/components/SideMenu'
...
function TestPaymentMethodResult() {
  let id = useId()

  const successfulCardTest = {
    card_number: '5299 9199 9999 9999',
    expiry: '12/99',
    cvc: '123',
  }

  const rejectedCardTest = {
    card_number: '4984 9199 9999 1234',
    expiry: '12/29',
    cvc: '123',
  }

  const copyToClipboard = (e: React.MouseEvent, card: TestCard) => {
    e.preventDefault()
    e.stopPropagation()
  ...
  return (
    ...
    <li
      ...
      onClick={(e) => copyToClipboard(e, successfulCardTest)}
    >
      <div
        id={`${id}-title`}
        className="flex items-center justify-between text-sm font-medium text-zinc-900 dark:text-white"
      >
        <span className="group-hover:text-blue-500 group-active:text-green-500 dark:group-active:text-green-400 transition-colors duration-150">Test card: succesful payment</span>
        <span className="text-xs text-zinc-500 group-hover:text-blue-500 group-active:text-green-500 dark:group-active:text-green-400 transition-colors duration-150">Click to copy</span>
      </div>
      <div className="mt-1 text-2xs text-zinc-500">
        {`Card: ${successfulCardTest.card_number} | Expiry: ${successfulCardTest.expiry} | CVC: ${successfulCardTest.cvc}`}
      </div>
    </li>
    <li
      className={clsx(
      ...
      aria-labelledby={`${id}-title`}
      onClick={(e) => copyToClipboard(e, rejectedCardTest)}
    >
      <div
        id={`${id}-title`}
        className="flex items-center justify-between text-sm font-medium text-zinc-900 dark:text-white"
      >
        <span className="group-hover:text-blue-500 group-active:text-green-500 dark:group-active:text-green-400 transition-colors duration-150">Test card: rejected payment</span>
        <span className="text-xs text-zinc-500 group-hover:text-blue-500 group-active:text-green-500 dark:group-active:text-green-400 transition-colors duration-150">Click to copy</span>
      </div>
      <div className="mt-1 text-2xs text-zinc-500">
        {`Card: ${rejectedCardTest.card_number} | Expiry: ${rejectedCardTest.expiry} | CVC: ${rejectedCardTest.cvc}`}
      </div>
    </li>
    </>
  )
...
function SearchResults({
  ...
  // Filter items based on pathname for test payment methods
  const filteredItems = collection.items.filter(result => {
    if (result.url.includes('test-payment-methods')) {
      if (!pathname) return false
      if (pathname.startsWith('/sdk')) {
        return result.url.startsWith('/sdk')
      } else if (pathname.startsWith('/products')) {
        return result.url.startsWith('/products')
      } else {
        return result.url.startsWith('/api')
      }
    }
    return true;
  })

  // Check if any result contains test-payment-methods
  const hasTestPaymentMethods = filteredItems.some(result => 
    result.url.includes('test-payment-methods#test-payment-methods')
  )

  console.log(filteredItems, hasTestPaymentMethods)

  return (
    <ul {...autocomplete.getListProps()}>
      {/* Render the special test payment method result if applicable */}
      {hasTestPaymentMethods && (
        <TestPaymentMethodResult />
      )}
      {/* Render regular results */}
      {filteredItems.map((result, resultIndex) => (
        <SearchResult
          key={result.url}
          result={result}
