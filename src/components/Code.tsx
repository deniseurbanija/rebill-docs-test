...
function CodePanelHeader({ tag, label }: { tag?: string; label?: string }) {
  ...
  return (
    <div className="flex h-9 items-center gap-2 border-y border-b-white/7.5 border-t-transparent bg-white/2.5 bg-zinc-900 px-4 dark:border-b-white/5 dark:bg-white/1 whitespace-nowrap">
      {tag && (
...
export function CodePanel({
  ...
  let { code, language = 'javascript' } = contentProps;
  if (language === 'netCode') {
    language = 'csharp';
  }
  ...
  // Assuming this code snippet is part of a React functional component
  if (!code) {
    ...
    return (
      <div className="group dark:bg-white/2.5">
        ...
        <div className="relative">
          <pre className={`overflow-x-auto p-4 text-xs text-white language-${language}`} style={{background: "none"}}>
            <code>{code}</code>
          </pre>
...
export function CodeGroup({
  ...
  if (userData?.publicKey && isValidElement(children) && children?.props?.code) {
    const code = children.props.code
    if(code) {
      children.props.code = code?.replaceAll('{YOUR_PUBLIC_KEY}', userData.publicKey)
    }
  }
